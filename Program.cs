...
namespace LogicsSS
{
    ...
    public class Program
    {
        public static void Main()
        {
            ...
            validationString.Validation("Enter You Phone Number");
    ...
    class UXs
    {
        ...
        private void Saver(string name, string info)
        {
            ...
            switch (name)
            {
                ...
                case "Buy":
                    if(true)
                    {
                        foreach (Product product in _USER._UserBin)
                        {
                            _USER.History($"Purchase: {product._ProductName}    Cost: {product._ProductCost}" +
                            $"({DateTime.Now.ToString("HH:mm:ss")})");
                        }
                    }
                    else 
                    {
                        _USER.History($"No enougt money:    " +
                        $"({DateTime.Now.ToString("HH:mm:ss")})");
                    }
                    break;
            }
        ...
        {
            string ValeraJsonInfo = JsonConvert.SerializeObject(_USER, Formatting.Indented);
            Console.WriteLine(ValeraJsonInfo);
        }
    }
    ...
    class User
    {
        ...
        public bool BuyProduct()
        {
            ...
            foreach (Product product in _UserBin)
            {
                Console.WriteLine($"Name: {product._ProductName}    Cost: {product._ProductCost}");
            }
            if (TempCost <= _OnCard)
            { 
                _OnCard -= TempCost;
...
{
    public void Validation(string MessageInWriteLine)
    {
        while (true)
        {
            Console.WriteLine(MessageInWriteLine + " ");
            string ValidMessage = Console.ReadLine();
            try
            {
                Int64 ParsedValue = Int64.Parse(ValidMessage);
                if (ValidMessage.Count() == 11)
                {
                    Console.WriteLine("Succes");
                    Console.WriteLine(ParsedValue.GetType());
                    SaveParsedNum(ParsedValue);
                    return;
                }
            }
            catch
            {
                if (ValidMessage.Count() != 10)
                {
                    goto ErrorUncorrectValue;
                }
                goto ErrorUncorrectType;
            }
            ErrorUncorrectType:
            {
                Console.WriteLine(ValidMessage.Count());
                Console.WriteLine($"This Message == {ValidMessage.GetType()} " +
    ...
    private void SaveParsedNum(Int64 ParsedNum)
    {
        Console.WriteLine($"{ParsedNum} saved in your Phone Number");
    }
}
